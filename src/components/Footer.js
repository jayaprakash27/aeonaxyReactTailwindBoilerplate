...
const menu_items = [
  ...
  {
    category: "Support",
    items: ["Help Center", "Video Tutorials", "Cookie Settings"],
  },
  {
    category: "Add-Ons",
  ...
  return (
    <div className=" w-screen p-10 bg-white flex flex-col-reverse md:flex-row">
      <div className=" w-full mt-6 md:w-1/3 flex items-start flex-col">
        ...
        <div className=" flex cursor-pointer w-full text-2xl justify-center md:justify-start my-5 gap-4">
      ...
      <div className=" w-full md:w-2/3 grid grid-cols-3 gap-4">
        {menu_items.map((item) => (
          <div key={item.category} className=" flex flex-col gap-3">
            <a
              href="#"
              className=" text-blue-950 hover:text-blue-800 text-xl font-bold"
            >
              {item.category}
            </a>
            {item.items.map((item) => (
